...
def navigateToWaypoint():
    ...
    while True:
        ...
        if len(points) >1:
            ...
            points.append([x,y,np.tan(y/float(x))])
        
        lvecx=points[len(points)-1][0] - points[len(points)-2][0] 
        lvecy=points[len(points)-1][0] - points[len(points)-2][0]
        cons=points[len(points)-1][1] - (float(y)/float(x))*points[len(points)-1][0]
